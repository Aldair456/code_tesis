import logging
import json
from typing import Dict, Any
from common.exceptions.handler_exception import handle_exception
from common.exceptions.exceptions import BadRequestError, NotFoundError
from services.service_credit_proposal_coril.src.config.dependencies import get_business_analysis_service

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

business_analysis_service = get_business_analysis_service()


def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    """
    evento 
    Handler Lambda para analizar un negocio (sector y producto). Invocado desde Step Function.
    Event: business_id (requerido), credit_memo_id (opcional).
    """
    request_id = getattr(context, "aws_request_id", "unknown")
    logger.info("Inicio análisis de negocio - Request ID: %s", request_id)

    try:
        business_id = event.get("business_id")
        if not business_id:
            raise BadRequestError("business_id es requerido.")
        credit_memo_id = event.get("credit_memo_id")
        user_id = (event.get("user") or {}).get("sub") or "system"

        logger.info("Analizando business_id: %s", business_id)
        if not business_analysis_service.is_available():
            raise BadRequestError("Servicio de análisis no disponible. Verifique configuración de API.")
        result = business_analysis_service.analyze_business_by_id(business_id, credit_memo_id=credit_memo_id)

        return {
            "business_id": business_id,
            "analysis": {
                "sector_economico": {
                    "contenido": result.sector_analysis,
                    "caracteres": len(result.sector_analysis),
                },
                "producto_demanda_mercado": {
                    "contenido": result.product_analysis,
                    "caracteres": len(result.product_analysis),
                },
            },
            "metadata": {
                "analizado_por": user_id,
                "servicio": business_analysis_service.get_service_info(),
                "request_id": request_id,
            },
            "request_id": request_id,
        }
    except (BadRequestError, NotFoundError):
        raise
    except Exception as err:
        logger.error("Error en lambda_handler: %s - Request ID: %s", err, request_id, exc_info=True)
        return handle_exception(err, request_id)


if __name__ == "__main__":
    event = {"business_id": "833bb9fa-9213-46b7-b0f8-f170b8aa1023"}
    
    print("\n=== INICIANDO PRUEBA DE HANDLER ===")
    print(f"API Key disponible: {'SÍ' if business_analysis_service.is_available() else 'NO'}")
    print(f"Info servicio: {business_analysis_service.get_service_info()}")
    
    print("\nEjecutando lambda_handler de análisis de negocio...\n")
    result = lambda_handler(event, None)
    print("\n=== RESULTADO ===")
    import json
    print(json.dumps(result, indent=2, ensure_ascii=False))
