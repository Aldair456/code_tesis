...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    """
    Handler Lambda para obtener URL de descarga del archivo Word (DOCX)
    de una propuesta de crédito coril.
    ...
    logger.info(f"Inicio generación de URL de descarga Word coril - Request ID: {request_id}")
    ...
    try:
        ...
        logger.info(f"Generando URL de descarga Word para propuesta coril: {proposal_id} (expiración: {expiration}s)")
        
        # Obtener URL de descarga para Word
        download_info = credit_proposal_coril_service.get_download_url_word(
        ...
        return Response(
            ...
            body={
                "success": True,
                "message": "URL de descarga Word coril generada exitosamente",
                "data": download_info,
        ...
        logger.error(f"Error en lambda_handler (Word): {str(err)} - Request ID: {request_id}", exc_info=True)
...
if __name__ == "__main__":
    ...
    print("\nEjecutando lambda_handler (Word)...\n")
