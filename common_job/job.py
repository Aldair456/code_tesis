...
class CreateJobModel(BaseModel):
    job_name: str
    job_type: str  # EXTRAER financial statement, o algo asi los estados
    resource_table: str
    resource_id: UUID
    business_id: UUID
    evaluator_id: UUID
...
class JobService:
    def __init__(self, sqs_client: SQSClient, table: Optional[str] = None):
    ...
    def create_identifier(self, resource_id: UUID, job_type: str) -> JobIdentifier:

        return JobIdentifier(
            resource_id=resource_id,
            job_type=job_type,
            resource_table=self.table
        )
    ...
    def create_job(self, job_data: CreateJobModel) -> dict:
        ...
        try:
            ...
            response = self.sqs_client.send_message(
                ...
                message_group_id=f"job-{job_data.job_type}"
            )
    ...
    def update_job(self, job_identifier: JobIdentifier, job_data: UpdateJobModel) -> dict:
        ...
        try:
            ...
            response = self.sqs_client.send_message(
                message=message_dict,
                message_group_id=f"job-update-{job_identifier.resource_id}"
            )

            return {
                "success": True,
                "message_id": response.get('MessageId', str(message.message_id)),
                "resource_id": str(job_identifier.resource_id),
                "job_type": job_identifier.job_type,
    ...
    def start_job(
        ...
        """Marca un job como en ejecución (RUNNING) y registra started_at."""
        ...
        if result_data is not None:
            update_fields["result_data"] = result_data
        update_data = UpdateJobModel(**update_fields)
        return self.update_job(job_identifier, update_data)

    def complete_job(self, resource_id: UUID, job_type: str, resource_table: Optional[str] = None,
    ...
    def increment_retry(self, resource_id: UUID, job_type: str, resource_table: Optional[str] = None,
