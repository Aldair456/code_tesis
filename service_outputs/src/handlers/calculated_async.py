...
def process_single_statement(statement_id, request_id):
    ...
    is_calculated = output_service.calculate_outputs(statement_id=statement_id)

    if is_calculated:
        logger.info(f"Outputs calculados correctamente para statement_id: {statement_id}")
    else:
        logger.warning(f"Outputs no se calcularon correctamente para statement_id: {statement_id}")
...
def process_sqs_event(event, request_id):
    ...
    for index, record in enumerate(records):
        ...
        try:
            ...
            results.append({
                "message_id": message_id,
                "statement_id": payload.get("statement_id") if 'payload' in dir() else None,
                "success": False,
                "error": str(err)
            })
...
def lambda_handler(event, context):
    ...
    logger.info(f"Inicio procesamiento cálculo de outputs - Request ID: {request_id}")
...
if __name__ == "__main__":
    ...
    event_sqs = {
        "Records": [
            ...
            {
                "messageId": "msg-003",
                "body": '{"statement_id": "99999999-9999-9999-9999-999999999999"}'
            }
        ]
    }
