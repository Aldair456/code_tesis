...
# Importaciones de clientes y repositorios
...
from models.model_1.ia.src.utils.helper_functions import (
    extract_delimited_text,
    textract_tables_to_text,
    find_page_content
)
from models.model_1.ia.src.utils.promts import (
...
class FinancialStatementIaService:
    ...
    def _job_run(self, job_id: str):
        ...
        try:
            ...
            response = self.job_service.start_job(UUID(job_id))
    ...
    def _job_complete(self, job_id: str, result_data: Dict[str, Any] = None):
        ...
        try:
            ...
            response = self.job_service.complete_job(UUID(job_id), result_data=result_data)
    ...
    def _fetch_accounts_by_type(self, account_type: str) -> List[Dict]:
        """
        Obtiene el diccionario contable desde account_extracts (mismo catálogo que subir_bd).
        """
        ...
        logger.info("Consultando account_extracts con type=%s", account_type)
        accounts = self.account_repository.find_all_by_filters(type=account_type)

        if not accounts:
            logger.warning(f"No se encontraron cuentas en account_extracts para type='{account_type}'.")
        else:
            logger.info("account_extracts type=%s: %s cuentas para el prompt IA", account_type, len(accounts))
